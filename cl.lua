--SonnyBee QBCore TODO
--Convert to new CreateThreads
--Kill all Citizens
--Convert to QBCore.Functions 
--Check Resmon
--Check Profiler
--Be sure that DRIFT is feelin GOOD
...
function ToggleDrift(vehicle)
	...
	if driftMode then
		...
		DrawNotif("~y~TCS~s~, ~y~ABS~s~, ~y~ESP ~s~je ~g~Zapnuté~s~!\nVozidlo je ve standardním režimu!")
	else
		
		PrintDebugInfo("drift")
		DrawNotif("~y~TCS~s~, ~y~ABS~s~, ~y~ESP ~s~je ~r~Vypnuté~s~!\nUžijte si jízdu bokem!")
	end
